...
/**
 * 
 * @ClassName: ExcelInputMetaInjection
 * @Description: To keep it simple, this metadata injection interface only
 *               supports the fields in the spreadsheet for the time being.
 * @author dongpo.jia
 * @date 2016年12月12日
 *
 */
public class ExcelInputMetaInjection implements StepMetaInjectionInterface {
	...
	public List<StepInjectionMetaEntry> getStepInjectionMetadataEntries()
		...
		// Add the fields...
		{
			StepInjectionMetaEntry fieldsEntry = new StepInjectionMetaEntry(
					Entry.FIELDS.name(), Entry.FIELDS.getValueType(),
					Entry.FIELDS.getDescription());
			all.add(fieldsEntry);

			StepInjectionMetaEntry fieldEntry = new StepInjectionMetaEntry(
					Entry.FIELD.name(), Entry.FIELD.getValueType(),
					Entry.FIELD.getDescription());
			fieldsEntry.getDetails().add(fieldEntry);

			for (Entry entry : Entry.values()) {
				if (entry.getParent() == Entry.FIELD) {
					StepInjectionMetaEntry metaEntry = new StepInjectionMetaEntry(
							entry.name(), entry.getValueType(),
		...
		// And the sheets
		{
			StepInjectionMetaEntry sheetsEntry = new StepInjectionMetaEntry(
					Entry.SHEETS.name(), Entry.SHEETS.getValueType(),
					Entry.SHEETS.getDescription());
			all.add(sheetsEntry);

			StepInjectionMetaEntry sheetEntry = new StepInjectionMetaEntry(
					Entry.SHEET.name(), Entry.SHEET.getValueType(),
					Entry.SHEET.getDescription());
	...
	public void injectStepMetadataEntries(List<StepInjectionMetaEntry> all)
		...
		for (StepInjectionMetaEntry lookFields : all) {
			...
			if (fieldsEntry != null) {
				if (fieldsEntry == Entry.FIELDS) {
					for (StepInjectionMetaEntry lookField : lookFields
							.getDetails()) {
						Entry fieldEntry = Entry.findEntry(lookField.getKey());
						if (fieldEntry != null) {
							if (fieldEntry == Entry.FIELD) {
								ExcelInputField inputField = new ExcelInputField();
								List<StepInjectionMetaEntry> entries = lookField
										.getDetails();
								for (StepInjectionMetaEntry entry : entries) {
									Entry metaEntry = Entry.findEntry(entry
											.getKey());
									if (metaEntry != null) {
										...
										case NAME:
											...
											break;
										case TYPE:
											inputField.setType(ValueMeta
													.getType(value));
											break;
										case LENGTH:
											inputField.setLength(Const.toInt(
													value, -1));
											break;
										case PRECISION:
											inputField.setPrecision(Const
													.toInt(value, -1));
											break;
										case CURRENCY:
										...
										case FORMAT:
											...
											break;
										case TRIM_TYPE:
											inputField.setTrimType(ValueMeta
													.getTrimTypeByCode(value));
											break;
										case REPEAT:
											//TODO add by dongpo.jia 暂时不使用此变量
											if(value==null) value="false";
							
											inputField
													.setRepeated(ValueMeta
								...
								for (StepInjectionMetaEntry entry : entries) {
									...
									if (metaEntry != null) {
										...
										case SHEET_START_ROW:
											startRow = Const.toInt(value, 0);
											break;
										case SHEET_START_COL:
											startCol = Const.toInt(value, 0);
											break;
										}
									}
								}

								sheets.add(new ExcelInputSheet(sheetName,
										startCol, startRow));
							}
						}
					}
		...
		//
		meta.setField(excelInputFields
				.toArray(new ExcelInputField[excelInputFields.size()]));
	...
	private enum Entry {
		...
		FIELDS(ValueMetaInterface.TYPE_NONE,
				"All the fields on the spreadsheets"), FIELD(
				ValueMetaInterface.TYPE_NONE, "One field"),

		NAME(FIELD, ValueMetaInterface.TYPE_STRING, "Field name"), TYPE(FIELD,
				ValueMetaInterface.TYPE_STRING, "Field data type"), LENGTH(
				...
				FIELD, ValueMetaInterface.TYPE_STRING,
				"Field trim type (none, left, right, both)"), FORMAT(FIELD,
				ValueMetaInterface.TYPE_STRING, "Field conversion format"), CURRENCY(
		...
		SHEETS(ValueMetaInterface.TYPE_NONE,
				"All the sheets in the spreadsheets"), SHEET(
