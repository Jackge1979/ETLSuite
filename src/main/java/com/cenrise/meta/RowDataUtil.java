...
/**
 * 
 * We use this class to do row manipulations like add, delete, resize, etc. That
 * way, when we want to go for a metadata driven system with hiding deletes,
 * over sized arrays etc, we can change these methods to find occurrences.
 * 
 *
 */
public class RowDataUtil {
	...
	/**
	 * Allocate a new Object array. However, over allocate by a constant factor
	 * to make adding values faster.
	 * 
	 * @param size
	...
	/**
	 * Resize an object array making it bigger, over allocate, return the
	 * original array if there's enough room.
	 * 
	 * @param objects
	 * @param newSize
	...
	/**
	 * Resize an object array making it bigger, over allocate, always create a
	 * copy of the original array, even if there's enough room in the old one.
	 * 
	 * @param objects
	 *            the original row
	 * @param newSize
	...
	/**
	 * This method concatenates data from an array of rows, each with their own
	 * specific length.
	 * 
	 * @param objects
	 * @param lengths
	...
	/**
	 * Remove an item from an Object array. This is a slow operation, later we
	 * want to just flag this object and discard it at the next resize. The
	 * question is of-course if it makes that much of a difference in the end.
	 * 
	 * @param objects
	 * @param index
	 * @return
	...
	public static Object[] removeItem(Object[] objects, int index) {
		...
		System.arraycopy(objects, index + 1, newObjects, index, objects.length
				- index - 1);
		return newObjects;
	}
	 ...
	 *            The first array
	 * @param the
	 *            length of the row data or of it's longer, the location of the
	 *            new extra value in the returned data row
	 * @param two
	 *            The second array
	 * @return a new Array containing all elements from one and two after one
	 *         another
	 */
	public static Object[] addRowData(Object[] one, int sourceLength,
			Object[] two) {
	 ...
	 */
	public static Object[] addValueData(Object[] rowData, int length,
			Object extra) {
	 ...
	 * @param index
	 *            the index of all the items in the source table to remove. We
	 *            don't check if the same index gets deleted twice!
	 */
